...
/**
 * Tax calculation helper for the Moodec storefront.
 *
...
class tax {
    ...
    public static function resolve_rate(?string $country): float {
        if (!get_config('local_moodec', 'tax_enable')) {
            return 0.0;
        }

        $rate = (float) get_config('local_moodec', 'tax_rate');

        $overridesjson = (string) get_config('local_moodec', 'taxcountryoverrides');
    ...
    public static function split(float $price, float $rate): array {
        $mode = (string) get_config('local_moodec', 'tax_mode');
